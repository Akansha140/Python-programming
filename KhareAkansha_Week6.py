...
def exercise2(df):
    a=df.idxmin().values
    b=df.idxmax().values
    df1=pd.DataFrame([a,b],columns=['Base Pay','Overtime Pay','Other Pay','Benefits'],index=['minloc','maxloc'])
    new_df = df.describe(percentiles=[])
    new_df=new_df.drop(['count','50%'],axis=0)
    new_df=new_df.append(df1,ignore_index=False)
    return new_df
...
# data
def exercise3(df):
    new_df = df.apply(lambda x: pd.Series(x.nlargest(3).index))
    new_df.index=['1st','2nd','3rd']
