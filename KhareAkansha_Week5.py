...
def exercise1(df, rowinds, colinds):
    df_subset = df.iloc[rowinds,colinds]
    return df_subset
...
def exercise2(df, col, output_label, val1, val2):
    new_df = df.loc[(col.values > val1) & (col.values < val2), [output_label]]
    return new_df
...
def exercise3(df):
    leng = len(df.carat)
    dist = np.empty((leng,leng))
    carat_i = df.carat
    table_i = df.table
    for i in range(len(df.carat)):
        for j in range(1,len(df.carat)):
            dist[i][j] = ((carat_i[i]- carat_i[j]) / 0.8)**2 + ((table_i[i] - table_i[j]) / 57)**2
    return dist
...
def exercise4(df):
    matx = exercise3(test_df_2)
    df['Dissimilarity'] = np.sum(matx,axis=0)
    print(df)
    return df
