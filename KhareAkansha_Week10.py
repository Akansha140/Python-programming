...
def exercise1(df, res_col):
    ...
    y = Series((df[res_col]-df[res_col].mean())/df[res_col].std())
    return X,y
...
def exercise3(X, y):
    col = []
    mod1 = DataFrame(X[X.columns[0]])
    mod2 = sm.OLS(y, mod1).fit()
    min_aic = mod2.aic
    col.append(X.columns[0])
    tempcol = list(X.columns[1:])
    for i in range(len(X.columns)-1):
        aic = []
        for j in (tempcol):
            mod1.insert(mod1.shape[1],j,X[j])
            mod2 = sm.OLS(y,mod1,axis=1).fit()
            aic.append(mod2.aic)
            mod1.drop([j],axis=1,inplace=True)
        if(min_aic>min(aic)):
            min_aic = min(aic)
            k = aic.index(min_aic)
            col.append(tempcol[k])
            mod1.insert(mod1.shape[1],tempcol[k],X[tempcol[k]])
            tempcol.pop(k)
    return col
